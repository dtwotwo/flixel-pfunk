 ...
 */
@:allow(flixel.FlxG) class SoundFrontEnd {
	/**
	 ...
	 */
	public var volumeUpKeys = [PLUS, NUMPADPLUS];

	/**
	 * The keys to decrease volume (see FlxG.keys for the keys available).
	 * Default keys: - (and numpad -). Set to null to deactivate.
	 */
	public var volumeDownKeys = [MINUS, NUMPADMINUS];
	 ...
	 */
	public var muteKeys = [ZERO, NUMPADZERO];
	#end
	...
	@:noCompletion inline function get_soundTray() return FlxG.game.soundTray;
	#end
	...
	public function pause():Void {
		...
		for (sound in list.members) if (sound != null && sound.exists && sound.active) sound.pause();
	}
	...
	public function resume():Void {
		...
		for (sound in list.members) if (sound != null && sound.exists) sound.resume();
	}
	...
	public function destroy(forceDestroy = false):Void {
		...
		for (sound in list.members) if (sound != null && (forceDestroy || !sound.persist)) sound.destroy();
	}

	/**
	 * Toggles muted, also activating the sound tray.
	 */
	@:haxe.warning("-WDeprecated") public function toggleMuted():Void {
		muted = !muted;
	...
	public function showSoundTray(up:Bool = false):Void {
		#if FLX_SOUND_TRAY
		if (FlxG.game.soundTray != null && soundTrayEnabled) FlxG.game.soundTray.show(up);
		#end
	}

	function new() {
		#if FLX_SAVE loadSavedPrefs(); #end
	}
	 ...
	 */
	@:allow(flixel.FlxGame) function update(elapsed:Float):Void {
		...
		if (!FlxInputText.globalManager.isTyping) {
			...
			if (FlxG.keys.anyPressed(volumeUpKeys) || FlxG.keys.anyPressed(volumeDownKeys)) {
				final checkLastHold = Math.floor((holdTime - helper) * 10);
				holdTime += elapsed;
				final checkNewHold = Math.floor((holdTime - helper) * 10);
				if (holdTime > helper && checkNewHold - checkLastHold > 0)
					changeVolume((checkNewHold - checkLastHold) * (FlxG.keys.anyPressed(volumeUpKeys) ? volumeMult : -volumeMult));
			}
		}
		#end
	}

	@:allow(flixel.FlxGame) function onFocusLost():Void {
		if (music != null) music.onFocusLost();
		for (sound in list.members) if (sound != null) sound.onFocusLost();
	}

	@:allow(flixel.FlxGame)
	function onFocus():Void {
		if (music != null) music.onFocus();
		for (sound in list.members) if (sound != null) sound.onFocus();
	}
	...
	@:haxe.warning("-WDeprecated") function set_volume(volume:Float):Float {
